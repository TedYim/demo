package com.example.demo.kafka.utils;
...
public class KafkaUtils {
    ...
    public static void createTopic(String topic, int numPartitions, int numReplications, Properties properties) {
        ...
        try {
            if (!AdminUtils.topicExists(zkUtils, topic)) {
                ...
                System.err.println("主题 : " + topic + " 已经存在...");
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
     ...
     * @param numReplicasPerPartition 分区副本分配的策略
     ...
     */
    public static void addPartitions(String brokerList, String topic, int numAddPartitions, int numReplicasPerPartition) {

        List<PartitionInfo> partitionInfos = getTopicMetadata(brokerList, topic);

        StringBuilder sb = new StringBuilder();
        for (PartitionInfo info : partitionInfos) {
            Node[] replicas = info.replicas();//获取每个分区的副本数
        ...
        scala.collection.Map<Object, Seq<Object>> replicaAssign = AdminUtils.assignReplicasToBrokers(brokerMetadata,
                partitionInfos.size() + numAddPartitions, // 总分区数
